...
from unicex.enums import Exchange
from unicex.hyperliquid.exchange_info import ExchangeInfo
...
async def main() -> None:
    ...
    print("Spot tickers:")
    print(list(ExchangeInfo._tickers_info.keys()))
    print(f"Len spot: {len(ExchangeInfo._tickers_info)}")
    for symbol in ["UBTC", "UETH", "NFT", "HYPE"]:
        info = ExchangeInfo.get_ticker_info(symbol)
        print(f"{symbol}: {info}")

    rounded = ExchangeInfo.round_price("UBTC", 123456.123456789)
    print(rounded)
    rounded_2 = ExchangeInfo.round_quantity("UBTC", 1.123456789)
    print(rounded_2)
